...
// -------------------- SAFE in-memory storage (prevents tracking/storage breakage) --------------------
const memoryStorage = (() => {
  let store = {};
  return {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------- Supabase init (NO fetch monkey-patching) ----------
  if (!window.supabase || typeof window.supabase.createClient !== "function") {
    setStatus(
      "Supabase library did not load. Check the <script src='./supabase.min.js'> or CDN script tag.",
      "error"
    );
  ...
  // IMPORTANT:
  // - No window.fetch override (SES/lockdown extensions often break that).
  // - Use in-memory storage to avoid tracking prevention breaking local/session storage.
  ...
  async function login(email, password) {
    // No forced signOut / no storage clearing here.
    // Those patterns are what often re-break login under strict environments.
    const warnTimer = setTimeout(() => {
      setStatus("Still logging in… (network/project may be slow)", "error");
    }, 8000);
  ...
  async function fetchCloseOnDate(tickerWithExchange, dateYYYYMMDD) {
    ...
    const data = await eodhdFetchJson(url);
    if (!Array.isArray(data) || data.length === 0) return { close: null, raw: data };
  ...
  function wireTxHandlers() {
    ...
    cancelBtn?.addEventListener("click", () => resetTxForm());
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        saveBtn.textContent = txState.editingId ? "Updating…" : "Saving…";
        ...
        const payload = {
          ...
          // store raw payloads if you want them
        ...
        if (txState.editingId) {
          res = await client.from(TX_TABLE).update(payload).eq("id", txState.editingId).select().single();
        } else {
          res = await client.from(TX_TABLE).insert(payload).select().single();
        }
        ...
        if (saveBtn) {
          ...
          saveBtn.textContent = txState.editingId ? "Update transaction" : "Save transaction";
        }
      }
    });
  ...
  async function loadTransactionsIntoTable() {
    ...
    const res = await client
      .from(TX_TABLE)
      .select(
        "id, isin, trade_date, side, quantity, unit_price, transaction_fee, description, eodhd_code, eodhd_exchange, instrument_currency, instrument_name, close_price"
      )
